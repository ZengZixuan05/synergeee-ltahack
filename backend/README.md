...
DataMall integration, a OneMap-based multi-modal journey planner, and live
data.gov.sg weather data. It has its own
...
- `src/routes/` - API route handlers: `GET /health`, `GET /api/lta/status`, `GET /api/transport/facilities`,
  ...
  `GET /api/weather/{forecast,rainfall}`
...
- `src/models/` - Domain models: canonical rail lines, canonical station shape, `TransportEvent` union, geospatial records, crowding types, bus reference/load types, journey planning types, weather types
- `src/services/` - Per-endpoint adapters (schema → normalisation → cached service): `facilitiesMaintenance/`,
  ...
  `weather/{twoHourForecast,rainfall}/`
...
- `src/weather/` - `WeatherClient` for data.gov.sg's real-time weather APIs (no auth needed at all — a third, independent integration)
...
- `docs/WEATHER_INTEGRATION.md` - The data.gov.sg weather integration and its known limitations
...
architecture, [`docs/JOURNEY_PLANNING.md`](./docs/JOURNEY_PLANNING.md) for
route planning, and [`docs/WEATHER_INTEGRATION.md`](./docs/WEATHER_INTEGRATION.md)
for weather data — each with how to verify it end to end.
...
- Environment variables are shared at the root `.env` file (see
  ...
  from Google Cloud Secret Manager instead. The weather integration needs no
  credentials at all.
